...
// --- Characteristic Function (Heston + Merton) ---
...
// --- Option Pricing (Carr-Madan) ---
double price_option_core(double S0, double K, double T, double r, double V0, SVCJParams p, int is_call) {
    double alpha = 1.5;
    double k_log = log(K);
    double limit = 200.0;
    int N = 120; // Optimized for speed
    double eta = limit / N;
    double complex sum = 0.0 + 0.0 * I;
    
    // Calculate Call Price first via FFT/Cos method logic
    for (int j = 0; j < N; j++) {
        double v = j * eta;
    ...
    if (is_call) {
        return (call_price > 0.0) ? call_price : 1e-5;
    } else {
...
double run_filter(double* returns, int n, double dt, SVCJParams p, double* final_state) {
    double v = p.theta; 
    double log_lik = 0.0;
    double jump_drift_c = p.lambda * (exp(p.mu_j + 0.5 * p.sigma_j * p.sigma_j) - 1.0);
    
    for (int t = 0; t < n; t++) {
        double v_pred = v + p.kappa * (p.theta - v) * dt;
        if (v_pred < MIN_VOL) v_pred = MIN_VOL;

        double mu = (0.0 - 0.5 * v_pred - jump_drift_c) * dt;
...
SVCJResult optimize_svcj(double* returns, int n_ret, double dt,
    ...
    // Auto-init Theta if history exists
    if (n_ret > 10) {
        double sum = 0; 
