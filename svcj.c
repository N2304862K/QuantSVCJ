...
SVCJResult optimize_svcj(double* returns, int n_ret, double dt,
    ...
    double final_st[2] = {best_p.theta, 0};
    if (n_ret > 0) run_filter(returns, n_ret, dt, best_p, final_st);
    
    res.p = best_p;
    res.spot_vol = final_st[0];
    res.jump_prob = final_st[1];
    res.error = best_err;
    return res;
}